...
* 在讲锁请求之前，我必须先讲MDL_key，因为它贯穿MDL系统始终，无论是改造MDL还是使用MDL，都离不开对这个概念的理解。
* 既然叫key，它本质上是一个唯一标识符，用于精准定义一个数据库对象（如Table、Schema 等）。MDL_key通过一个三元组定义（namespace, db_name, name）：
    * namespace (命名空间）
    * db_name （数据库名）
    * name （表名/对象名）
> todo: db_name 有时会是空的，这个要说明一下
当你向 MDL 系统输入一个锁请求时，必须提供一个 MDL_key，以明确告知MDL系统你要锁定的究竟是哪个对象。

db_name和name好理解，namespace是个什么东西？

作为C++开发工程师，很容义联想到C++中的namespace，C++中它的作用是将全局命名空间分隔成多个逻辑上独立的部分，避免命名冲突。同样，理解此处mdl_namespace的核心也在于**冲突空间**的划分 —— 不同namespace的下的锁请求是不会冲突的。
...
### MDL_request 与 MDL_ticket
MDL 拥有清晰的生命周期管理机制，这一点在 MDL_request 和 MDL_ticket 的设计上体现得尤为直接：

* MDL_request（锁请求）： 由 MDL 系统外部控制。请求源自系统外部，此时将 MDL 系统视为一个“黑盒”，其分配与生命周期均由外部调用者负责。

* MDL_ticket（已授予的锁）： 由 MDL 系统内部控制。它是锁被授予后的凭证，产生于 MDL 系统内部，并由系统闭环管理。

一个“锁请求”与一个“已授予的锁”分别由不同的类表示，这意味着它们拥有不同的内存分配位置以及不同的生命周期。

初看这里的实现可能会觉得绕，但其实只要把握住核心原则——“哪里产生的，就由哪里控制”。

关于 MDL_ticket 的内部细节，我会在后文进行更详细的描述。


### MDL_lock
MDL_lock 的名字非常具有迷惑性，我们很容易认为这是一把锁（再一次），然而更准确地说它表示的是一个**锁状态池**。

在 MySQL 中会有很多的 MDL_lock 实例，也就是说会有很多的锁状态池。那么这些锁状态池如何标记和区分呢？这里就会使用到前文讲到的MDL_key。它们之间是一一对应的，通过MDL_key来进行唯一标识。如前文所讲，MDL_key 表示一个数据库对象，所以 **MDL_lock 是一个数据库对象的锁状态池子”。

不同的池子区分开了，那每一个池子里具体放的时什么呢？
* 池子里放的是该对象上的所有活动的锁请求状态（已授予或等待中）
* m_granted 双向链表
* m_waiting 双向链表

值得一提的是MDL_lock定义在mdl.cc中，这意味着它是MDL系统的内部类，外部模块不可以直接创建MDL_lock对象，MDL_lock的生命周期在MDL系统内部管理。
